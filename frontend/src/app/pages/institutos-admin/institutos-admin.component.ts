...
import { BitacoraDto, InstitucionDto, InstitucionesService } from '../../services/instituciones.service';
...
@Component({
  ...
  templateUrl: './institutos-admin.component.html',
  styleUrl: './institutos-admin.component.scss'
})
export class InstitutosAdminComponent implements OnInit {

  dataSource = new MatTableDataSource<InstitucionDto>([]);
  displayedColumns: string[] = ['id', 'nombre', 'estado', 'responsable', 'telefonos', 'correos', 'ultima_gestion', 'acciones'];
  isLoading = false;
  ...
  ngOnInit(): void {
	...
	cargarInstituciones() {
		this.isLoading = true;
		this.institucionesService.getAllAdmin().subscribe({
			next: (response) => {
